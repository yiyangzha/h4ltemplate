...
The lepton momentum scale, resolution, and efficiency distortions use the
original configured amplitudes.  In particular, no additional scale or
efficiency enlargement is applied beyond \texttt{config.json}.  Branches with
explicit energy values are scaled by the same ratio as the corresponding
transverse momentum, so the energy response follows the configured momentum
distortion without introducing an independent mass-shape change.
...
momentum scale and resolution changes.  The modified mass peak can shift
relative to the raw sample because the workflow intentionally applies enlarged
scale changes for validation visibility.
