...
- [x] [D2][VT9][VT10] Use a binned simultaneous `pyhf`/HistFactory-style
...
- [x] [D5][VT9][FIG3] Use one global `mu` scaling all Higgs production
...
- [x] [D9][VT7][VT12][FIG3] Address method parity through a binding
  ...
  before downgrade. Proof: `analysis_note/results/expected_mass_scan.json` records expected shifted-template closure, and `analysis_note/results/observed_mass_scan.json` records the full-data observed shifted-template mass scan with `mu` profiled at each mass hypothesis. Both are approximated detector-level evidence, not official calibrated CMS-quality mass measurements.
...
- [x] [D9][SP5][VT12] Lepton momentum scale and resolution shape uncertainty
...
- [x] [FIG6][SP8] Higgs branching-fraction uncertainty if converting `mu` to a
...
- [x] [SP9] qqZZ and ggZZ background normalization and shape uncertainties.
...
- [D] [D7][D8][SP12] Classifier/NN input modeling and category migration
...
- [x] [D9] Method-parity cross-check: compare the nominal binned `pyhf`
  ...
  reason. Proof: `analysis_note/results/expected_mass_scan.json` contains expected shifted-template closure, and `analysis_note/results/observed_mass_scan.json` contains the full-data observed category-simultaneous mass-profile scan with `mu` profiled.
- [x] [D9][VT8] Signal injection/recovery at `mu = 0`, `1`, `2`, and `5`; bias
  ...
  and fitted `mu`.
- [x] [D2][VT9] Combined and per-category goodness-of-fit with chi2/ndf and
...
- [x] [FIG2][D4] Prefit/postfit `m4l` distributions in final-state categories
  and any validated classifier categories.
- [x] [FIG3][D5][D9] Signal-strength likelihood scan for `mu` and validated
...
- [x] [FIG5][D2][SP*] Systematic impact ranking and uncertainty breakdown on
  `mu`.
- [x] [FIG6][REF-MATRIX] Comparison summary figure/table against
...
- [x] [D4][D5] Final-state channel compatibility: 4e, 4mu, and 2e2mu fitted
