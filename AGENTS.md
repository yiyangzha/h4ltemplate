...
for each inference+doc pair in [(4a, Doc 4a), (4b, Doc 4b), (4c, Doc 4c)]:
  ...
  5. CHECK — read arbiter verdict. Handle ITERATE/ESCALATE
     ...
     ITERATE → fixer agent applies fixes →
       ...
       each finding FIXED or NOT FIXED with evidence." Opus/5.5 model —
...
- Spawning subagents with the wrong model tier. Read `TOGGLES.md` for the
  ...
  for per-role assignments. Never downgrade a role that requires Opus/5.5
  (executor, physics reviewer, arbiter) to Sonnet/5.4 or Haiku/5.3.
...
all subagents use `model: "opus/5.5"`. Principle: Opus/5.5 (high) for physics reasoning
and final judgment; Sonnet/5.5 (medium) for visual critique and secondary review;
Haiku/5.5 (low) for fast mechanical tasks.
