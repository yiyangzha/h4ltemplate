...
A shifted-template detector-level mass scan with $\mu$ profiled finds best grid point $m_H=125.0$ GeV; this scan is reported only as method-parity evidence and is not promoted to an official CMS-quality Higgs mass measurement.
\end{abstract}
\tableofcontents
\clearpage
\section*{Change Log}
...
\item Added the observed shifted-template mass scan with $\mu$ profiled at each grid point, explicitly classified as detector-level diagnostic evidence rather than an official calibrated mass measurement.
\end{itemize}
...
The observed shifted-template mass scan uses the broad likelihood for observed counts and background constraints, while excluding the Z-peak region from Higgs mass hypotheses. It profiles $\mu$ at each mass grid point and finds a best grid value of $m_H=125.0$ GeV with profiled $\mu=2.4776$. The reported interval is $[125.0,125.0]$ GeV only by the coarse $\Delta(-2\ln L)\le1$ grid criterion and is not meaningful as a calibrated uncertainty.
...
Quantity & Value \\
\midrule
Scan range & 110--150 GeV in 2.5 GeV steps \\
Best grid point & 125.0 GeV \\
Best profiled $\mu$ & 2.4776 \\
Grid interval & [125.0, 125.0] GeV, not meaningful \\
...
\caption{Observed shifted-template detector-level mass scan with $\mu$ profiled at each mass hypothesis. The best grid point is 125.0 GeV, but the scan is not an official CMS-quality mass measurement because independent mass-hypothesis MC and official calibration/morphing inputs are unavailable.}
...
The observed mass-scan best grid point is 125.0 GeV. CMS-HIG-16-041/JHEP 11 (2017) 047 reports $m_H=125.26\pm0.20\mathrm{(stat)}\pm0.08\mathrm{(syst)}$ GeV, giving a quadrature total uncertainty of 0.215 GeV \cite{CMS-HIG-16-041}. The PDG 2024 world-average context value is $125.20\pm0.11$ GeV \cite{PDG2024}. The grid residuals are -0.26 GeV and -0.20 GeV, respectively, but no calibrated pull is quoted for this analysis because the scan has only a coarse 2.5 GeV grid and no meaningful statistical/systematic split.
...
Mass diagnostic & 125.0 GeV best grid; visual grid half-step 1.25 GeV, not stat/syst & CMS-HIG-16-041: $125.26\pm0.20\mathrm{(stat)}\pm0.08\mathrm{(syst)}$ GeV & residual -0.26 GeV; no calibrated pull quoted \\
Mass diagnostic & 125.0 GeV best grid; visual grid half-step 1.25 GeV, not stat/syst & PDG 2024: $125.20\pm0.11$ GeV world average & residual -0.20 GeV; no calibrated pull quoted \\
...
\caption{Full-data signal-strength and mass-diagnostic comparison with public CMS and PDG references. The labels state the center-of-mass energy and luminosity for dataset-specific entries; the PDG entry is labeled as a world average rather than a dataset. The signal-strength pulls are 1.79 standard deviations relative to CMS-HIG-16-041 and 1.96 standard deviations relative to CMS-HIG-19-001. In the mass panel, CMS-HIG-16-041 shows separate statistical and systematic uncertainty bars plus the total uncertainty, while this analysis is shown with a coarse grid half-step only to emphasize that the shifted-template scan is diagnostic and not a calibrated mass measurement.}
...
\caption{BDT classifier ROC curve for the S2 attempt. The repaired broad-window MVA is diagnostic rather than random, but S2 is not promoted because score-shape, category viability, and mass-sculpting gates fail.}
...
\caption{Tuned BDT classifier ROC curve for the S2 attempt. The repaired broad-window training demonstrates useful separation, but the active result remains S1 final-state categories because the full promotion gate is not satisfied.}
...
\caption{Logistic classifier ROC curve for the S2 attempt. This diagnostic baseline is retained for transparency; no classifier category enters the nominal likelihood.}
...
\caption{Small NN classifier ROC curve for the S2 attempt. The neural-network route is preserved as rejected-approach evidence and is not used to define the active signal-strength categories.}
